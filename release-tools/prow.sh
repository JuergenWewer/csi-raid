...
# - kind (https://github.com/JuergenWewer/kind) installed
...
# the desired version is downloaded from https://github.com/JuergenWewer/kind/releases
...
install_kind () {
    ...
    if run curl --fail --location -o "${CSI_PROW_WORK}/bin/kind" "https://github.com/JuergenWewer/kind/releases/download/${CSI_PROW_KIND_VERSION}/kind-linux-amd64"; then
        chmod u+x "${CSI_PROW_WORK}/bin/kind"
    else
        git_checkout https://github.com/JuergenWewer/kind "${GOPATH}/src/sigs.k8s.io/kind" "${CSI_PROW_KIND_VERSION}" --depth=1 &&
...
start_cluster () {
    ...
    if ! [ "$image" ]; then
        if ! ${csi_prow_kind_have_kubernetes}; then
            ...
            # Changing into the Kubernetes source code directory is a workaround for https://github.com/JuergenWewer/kind/issues/1910
    ...
    if tests_need_kind; then
        ...
        if ${CSI_PROW_BUILD_JOB}; then
            ...
            for i in $cmds; do
                ...
                # (https://github.com/JuergenWewer/kind/issues/328).
